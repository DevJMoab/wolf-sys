...
import { Home, FileText, Users, Settings, Calendar, BarChart2, Mail } from 'lucide-react'
...
export default function Sidebar({ isOpen, toggleSidebar, setCurrentPage }: SidebarProps) {
  ...
  return (
    <aside className={`bg-gray-800 dark:bg-gray-950 text-white transition-all duration-300 ease-in-out 
      ${isOpen ? 'w-64' : 'w-20'} flex flex-col h-full`}>
      
      {/* Logo */}
      <div className={`flex items-center justify-center p-4 border-b border-gray-700 ${isOpen ? 'justify-start' : 'justify-center'}`}>
            ...
            <li key={item.name}>
              <Link
                ...
                className={`flex items-center p-3 rounded-lg transition-colors 
                  ${pathname === item.href ? 'bg-amber-600 text-white' : 'hover:bg-gray-700'}`}
              >
      ...
      {/* Footer */}
      <div className={`p-4 border-t border-gray-700 text-center text-xs ${!isOpen && 'hidden'}`}>
        <p>© Dev Johan Moab</p>
